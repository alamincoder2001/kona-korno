				...
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>{{ Math.trunc(product.current_quantity / product.per_unit_convert) }} {{ product.converted_name }} {{ (product.current_quantity % product.per_unit_convert) }} {{ product.Unit_Name }}</td>
