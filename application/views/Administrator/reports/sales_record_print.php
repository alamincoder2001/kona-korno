...
<body style="background:none; margin-bottom: 30px;">
      <table width="800px" >
        <tr>
...
<?php if($invoive=="invoice" || $invoive=='All'|| $invoive=='User') { ?>
    <div class="" >
        <table class="border" cellspacing="0" cellpadding="0" width="90%">
            ...
            <?php $i = 0; $totalSales = 0; $TotalQty = 0; $totalcommision = 0; $totalpaid = 0; $TotalPaid= $TotalDue = $TotalSubTotal =0;
            ...
            if(isset($records) && $records){
                foreach($records as $record){
                    ...
                    foreach ($SDROWS as $SDROW) {
                        ...
                        } else{ $tbox="0 Box ".$SDROW->SaleDetails_TotalQuantity." Pcs"; }
                        ...
                        if($k==1){?>
                            <tr>
                                ...
                                <td style="text-align: right;" colspan="2"><?php echo ($SDROW->SaleDetails_TotalQuantity*$SDROW->SaleDetails_Rate)-$SDROW->Discount_amount ; ?></td>
                            </tr>
                        <?php } else{ ?>
                            <tr>
                                <td>&nbsp;</td>
                        ...
                        <?php } $Qty += $SDROW->SaleDetails_TotalQuantity; $TotalQty += $SDROW->SaleDetails_TotalQuantity; } ?>
                    <tr>
                        <td style="text-align: right;" colspan="4"></td>

                        <td style="text-align: center;"><strong> Total Quantity: <?php echo $Qty; ?></strong></td>
                        <td style="text-align: center;"><strong>Total Paid: <?php echo $record->SaleMaster_PaidAmount; $TotalPaid +=$record->SaleMaster_PaidAmount; ?> <br/>Total Due: <?php echo number_format($record->SaleMaster_DueAmount, 2); $TotalDue +=$record->SaleMaster_DueAmount;  ?></strong></td>
                        <td style="text-align: right;" colspan="2"><strong>Total: <?php echo number_format($record->SaleMaster_SubTotalAmount, 2); $TotalSubTotal +=$record->SaleMaster_SubTotalAmount;  ?></strong></td>
                    </tr>
                <?php  }  }?>
            <tr>
                ...
                <td style="text-align: center;"><strong>Total Paid: <?php echo number_format($TotalPaid,2); ?> <br/>Total Due: <?php echo number_format($TotalDue, 2); ?></strong></td>
                <td style="text-align: right;" colspan="2"><strong>Total: <?php echo number_format($TotalSubTotal, 2); ?></strong></td>
            </tr>
        </table>
    </div>
...
<?php }else{ ?>
    <div class="" style="table-responsive">
        <table class="border" cellspacing="0" cellpadding="0" width="90%">
            ...
            <?php $totalpurchase = 0; $Totalpaid = 0;
            if(isset($records) && $records){
                foreach($records as $record){
                    ...
                    <tr align="center">
                        ...
                        <td align="center"><a href="<?php echo base_url(); ?>Administrator/sales/sales_update_form/<?php echo $record->SaleMaster_SlNo; ?>" title="Update" style="color:green;"><span class="fa fa-pencil"></span></a></td>
                        <td><a style="cursor:pointer" onclick="window.open('<?php echo base_url();?>sales_record_print/<?php echo $record->SaleMaster_SlNo;?>', 'newwindow', `width=${screen.width}, height=${screen.height}`); return false;"> <i class="fa fa-print" style="font-size:24px;color:green"></i></a></td>
