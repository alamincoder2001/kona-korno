			...
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				<div class="form-group">
					...
					<select class="form-control" v-model="searchType" @change="onChangeSearchType">
						...
						<option value="a">Delivered Order</option>
					</select>
				</div>
						...
						<tr v-for="sale in sales">
							...
							<td style="text-align:center;">
								...
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									...
									<a href="" v-if="sale.Status != 'a'" title="Order Delivered" @click.prevent="deliveryOrder(sale.SaleMaster_SlNo)"><i class="fa fa-shopping-cart"></i></a>
	...
	new Vue({
		...
		data() {
			return {
				...
				sales: [],
				searchTypesForRecord: ['', 'customer', 'p', 'a'],
			}
		},
		methods: {
				...
				axios.get('/check_sale_return/' + sale.SaleMaster_InvoiceNo).then(res => {
					if (res.data.found) {
						...
						if (sale.is_service == 'true') {
							...
							location.replace('/order/' + sale.SaleMaster_SlNo);
						}
					}
				})
			},
			...
			getSalesRecord() {
				let filter = {
					...
					dateTo: this.dateTo,
					Status: this.searchType == 'p' ? 'p' : this.searchType == 'a' ? 'a' : '',
				}

				let url = '/get_orders';
				if (this.recordType == 'with_details') {
					url = '/get_order_record';
				}
